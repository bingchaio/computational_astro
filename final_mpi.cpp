...
int NRank, MyRank=0;				 // Rank for MPI                           
    ...
    if ( MyRank == 0){
    	for (int i = 0; i < Nx/2; i++) {
            for (int j = 0; j < Ny; j++) {
            	for (int k = 0; k < Nz; k++) {
                    rho[i][j][k] = 0.;
		    MPI_Send( &rho[i][j][k], 1, MPI_DOUBLE, TargetRank, Tag, MPI_COMM_WORLD );
                }
            }
        }
    }
    	...
    	for (int i = Nx/2; i < Nx; i++) {
            for (int j = 0; j < Ny; j++) {
            	for (int k = 0; k < Nz; k++) {
		    MPI_Recv( &rho[i-Nx/2][j][k], 1, MPI_DOUBLE, TargetRank, Tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
                    rho[i][j][k] = 0.;
		    if ( MyRank == 0 ) MPI_Recv( &rho[i][j][k], 1, MPI_DOUBLE, TargetRank, Tag+1, MPI_COMM_WORLD, MPI_STATUS_IGNORE );
    		    else MPI_Send( &rho[i][j][k], 1, MPI_DOUBLE, TargetRank, Tag+1, MPI_COMM_WORLD );
                }
