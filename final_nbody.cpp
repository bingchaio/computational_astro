...
void Get_Direct_Force_of_Particle(double * X, double *Y, double *Z, double x, double y, double z, double & F_x, double & F_y, double & F_z) {
    ...
    for (int i=0;i<n;i++){
	double d = pow(pow(X[i]-x,2)+pow(Y[i]-y,2)+pow(Z[i]-z,2),0.5);
	if (d!=0){
	   F_x += G*m*m/pow(d,3)*(X[i]-x);
	   F_y += G*m*m/pow(d,3)*(Y[i]-y);
	   F_z += G*m*m/pow(d,3)*(Z[i]-z);
	}
    }
}
    
void Get_Mesh_Force_of_Particle(double *** U, double x, double y, double z, double & F_x, double & F_y, double & F_z, int mode) {
