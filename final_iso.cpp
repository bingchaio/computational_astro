...
array3<Complex> rho_x(2*Nx,2*Ny,2*Nz,sizeof(Complex)); // rho for fft
array3<Complex> phi_x(2*Nx,2*Ny,2*Nz,sizeof(Complex)); // phi for fft
...
array3<Complex> R_x(2*Nx,2*Ny,2*Nz,sizeof(Complex)); 
array3<Complex> R_k(2*Nx,2*Ny,2*Nz,sizeof(Complex));
...
int main() {
    /* Variables */
    double * x = new double[2*n];          //positions of the particles
    double * y = new double[2*n];
    double * z = new double[2*n];
    double * vx = new double[2*n];         //velocities of the particles
    double * vy = new double[2*n];
    double * vz = new double[2*n];
    double *** rho = new double ** [2*Nx]; // mass density
    double *** U = new double ** [2*Nx];   // potential
    double *** W = new double ** [2*Nx];   // Poisson solver weighting matrix
    double *** R = new double ** [2*Nx];   //Isolated BC symmetric discrete Green’s function
    const int NThread = 4;          // number of threads
    omp_set_num_threads( NThread );
    ...
    for (int i = 0; i < 2*Nx; i++) {
        rho[i] = new double * [2*Ny];
        U[i] = new double * [2*Ny];
	W[i] = new double * [2*Ny];
	
        for (int j = 0; j < 2*Ny; j++) {
            rho[i][j] = new double[2*Nz];
            U[i][j] = new double[2*Nz];
	    W[i][j] = new double[2*Nz];
   ...
   for (int i = 1; i < Nx; i++) {
        for (int j = 1; j < Ny; j++) {
		   ...
		   R_x(i,j,k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5);
		   R_x(2*Nx-i,j,k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5); 
		   R_x(i,2*Ny-j,k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5);
		   R_x(i,j,2*Nz-k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5);
		   R_x(2*Nx-i,2*Ny-j,k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5);
		   R_x(2*Nx-i,j,2*Nz-k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5);
		   R_x(i,2*Ny-j,2*Nz-k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5);
		   R_x(2*Nx-i,2*Ny-j,2*Nz-k) = -1./pow((pow(i*dx,2)+pow(j*dy,2)+pow(k*dz,2)),0.5);
		}
	    }
        }
